...
return_data <- function(){
  ...
  master_values <- c(list(cbc_grouped),pa_flow,list(csf1r_brdu,csf1r_monos)) %>%
    ...
    mutate(
      ...
      animal_id,
      dpi,
      #no_mono = as.numeric(no_mono), there's a really strange bug that is converting factor to a percentile?
      no_mono = 1000 * as.numeric(as.character(no_mono)),
      cohort,
...
#write a function to calculate absolute number from master_values, please.
#actually that probably goes in the master script
