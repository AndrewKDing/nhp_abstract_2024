...
cbc_2024_06_13_14_21_56 <- read_excel("/Users/lilyceraso/Library/Containers/com.microsoft.Excel/Data/Downloads/cbc_2024-06-13_14-21-56.xlsx")
View(cbc_2024_06_13_14_21_56)
...
cbc_data <- cbc_2024_06_13_14_21_56[,c("Animal", "Sample Date", "#Neut", "#Lymph", "#Mono", "#Eosin", "#Baso")]
...
#modifying animals to pvl_ART_wk cohort
cbc_2024_wk_cohort <- c("JC80", "KC49", "MA93", "JL49", "JD87", "MP97", "LV26", "LA79")
cbc_2024_wk_cohort.df <- cbc_2024_dpi[cbc_2024_dpi$Animal %in% cbc_2024_wk_cohort, ]

#create separate data frames for each cell type
baso_data <- cbc_2024_wk_cohort.df[,c("Animal", "#Baso", "ART type", "DPI")]
baso_data$`#Baso` <- as.numeric(baso_data$`#Baso`)
mono_data <-cbc_2024_wk_cohort.df[,c("Animal", "#Mono", "ART type", "DPI")]
mono_data$`#Mono` <- as.numeric(mono_data$`#Mono`)
lymph_data <- cbc_2024_wk_cohort.df[,c("Animal", "#Lymph", "ART type", "DPI")]
lymph_data$`#Lymph` <- as.numeric(lymph_data$`#Lymph`)
neut_data <- cbc_2024_wk_cohort.df[,c("Animal", "#Neut", "ART type", "DPI")]
neut_data$`#Neut` <- as.numeric(neut_data$`#Neut`)
eosin_data <- cbc_2024_wk_cohort.df[,c("Animal", "#Eosin", "ART type", "DPI")]
